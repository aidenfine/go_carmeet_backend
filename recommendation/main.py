...
user_embedding = tf.keras.layers.Embedding(input_dim=len(df['user_id'].unique()) + 1, output_dim=embedding_dim)(user_input)
post_embedding = tf.keras.layers.Embedding(input_dim=len(df['post_id'].unique()) + 1, output_dim=embedding_dim)(post_input)
...
model.fit([train['user_id'], train['post_id']], train['interaction_value'], epochs=10, batch_size=16)
